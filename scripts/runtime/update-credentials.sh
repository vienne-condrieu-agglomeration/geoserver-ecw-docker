...
# Credits to https://github.com/meggsimum/geoserver-docker/, https://github.com/kartoza/docker-geoserver and https://github.com/geoserver/docker
...
if [ ! -d "${GEOSERVER_DATA_DIR}/security" ]; then
  cp -r "${JETTY_BASE}/webapps/geoserver/data/security" "${GEOSERVER_DATA_DIR}/"
fi
...
USERS_XML_ORIG="${JETTY_BASE}/webapps/geoserver/data/security/usergroup/default/users.xml"
echo "USING USERS XML ORIGINAL:" $USERS_XML_ORIG
ROLES_XML_ORIG="${JETTY_BASE}/webapps/geoserver/data/security/role/default/roles.xml"
...
CLASSPATH="$JETTY_BASE/webapps/geoserver/WEB-INF/lib/"
